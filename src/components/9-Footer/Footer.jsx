...
export default function Footer() {
  ...
  const GradientSquare = ({
    ...
    >
      <div
        className={`w-full h-full rounded-3xl ${rotation} transition-all duration-500 ${gradientClasses} blur-3xl animate-pulse-slow ${hoveredSquare === id ? `scale-105 ${hoverShadowClasses}` : ''
          }`}
      />
      <div
  ...
  return (
    <footer className="relative overflow-hidden bg-gradient-to-bl from-black via-gray-900 to-black bg-gradient-to-br from-black via-gray-900 to-black opacity-90 pt-16" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="absolute top-0 left-0 right-0 h-full overflow-hidden">
        <GradientSquare
          id={3}
          position="top-2/3 left-3/3"
          rotation=""
          gradientClasses="bg-gradient-to-r from-rose-700/20 via-fuchsia-600/20 to-indigo-00/20"
          hoverShadowClasses="border border-rose-500/30 shadow-[0_0_30px_-5px_rgba(244,63,94,0.5)]"
          overlayGradient="bg-gradient-to-tr from-rose-400/10 to-transparent"
        />
      </div>
      ...
      <div className="container mx-auto px-4 cairo relative">
              ...
              {quickLinks.map((link, index) => (
                <li key={index}>
                  <a
                    ...
                    className="group flex items-center  justify-center text-white hover:text-emerald-400 transition-colors duration-300"
                  >
        ...
        <div className="border-t border-gray-800/50 py-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center 2xl:gap-10 lg:gap-8 md:gap-4 ">
              {t('footer.terms', { returnObjects: true }).map((item, index) => (
                <a
                  key={index}
                  href="#"
                  className="text-md text-white hover:text-emerald-400 transition-colors duration-300"
                >
                  {item}
                </a>
              ))}
