      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
            ...
            <h3 className="font-semibold mb-2">
              {currentLang === 'ar' ? 'وقت التسليم' : 'Delivery Time'}
            </h3>
            <p>{currentLang === 'ar' ? project.deliveryTimeAr : project.deliveryTime}</p>
          </div>
  
          <div className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
            <Shield className="w-8 h-8 text-purple-500 mb-4" />
            <h3 className="font-semibold mb-2">
              {currentLang === 'ar' ? 'الضمان' : 'Warranty'}
            </h3>
            <p>{currentLang === 'ar' ? '6 أشهر' : '6 Months'}</p>
          </div>
        </div>
        ...
        <div className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
          ...
          <div className="space-y-4">
            {project.clientDeliverables?.map((deliverable, index) => (
              <div key={index} className="flex items-start gap-3 p-4 bg-blue-500/10 rounded-lg">
                {deliverable.icon}
                <div>
                  <h4 className="font-medium">
                    {currentLang === 'ar' ? deliverable.titleAr : deliverable.titleEn}
                  </h4>
                  <p className="text-sm opacity-80">
                    {currentLang === 'ar' ? deliverable.descriptionAr : deliverable.descriptionEn}
                  </p>
                </div>
              </div>
            ))}
  ...
  export const FeaturesSection = ({ project, currentLang, isDarkMode }) => (
    <div className="space-y-12">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {(currentLang === 'ar' ? project.features : project.featuresEn).map((feature, index) => (
          <motion.div
            key={index}
            initial={{ opacity: 0, y: 20 }}
...
FeaturesSection.propTypes = {
  ...
  export const TechnicalSection = ({ project, currentLang, isDarkMode }) => (
    <div className="space-y-12">
      <h2 className="text-3xl font-bold mb-8">
        {currentLang === 'ar' ? 'التفاصيل التقنية' : 'Technical Details'}
      </h2>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-800/50' : 'bg-white'} shadow-lg`}>
          ...
          <ul className="space-y-2">
            {(currentLang === 'ar' ? project.targetAudience : project.targetAudienceEn).map((item, index) => (
  ...
  export const ProcessSection = ({ project, currentLang, isDarkMode }) => (
    <div className="space-y-12">
      ...
      <div className="relative">
        ...
        {(currentLang === 'ar' ? project.process : project.processEn).map((step, index) => (
          <motion.div
            key={index}
            initial={{ opacity: 0, x: index % 2 === 0 ? -20 : 20 }}
            whileInView={{ opacity: 1, x: 0 }}
            className={`relative grid grid-cols-2 gap-8 mb-12 ${
